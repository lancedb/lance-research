...
data_dir <- "introductory/experiments/late_materialization/data/2023-12-29T15:09:32/"
# data_dir <- "../experiments/late_materialization/data/2023-12-29T15:09:32/"
...
#| fig-cap: Time to scan single column with filter. The `id` column is a incrementing id. The `vec` column is a 768-dimensional vector. The `img` column is a 4KB binary blob.
#| fig-height: 4
ggplot(runtime_results, aes(color=format, linetype=late_materialization, x=selectivity, y=runtime)) +
 geom_line() +
...
#| fig-cap: The total bytes read while scanning a single column with filter.
#| fig-height: 4
ggplot(io_results, aes(color=format, linetype=late_materialization, x=selectivity, y=total_bytes)) +
 geom_line() +
 facet_wrap(~ columns, scales = "free_y") +
...
#| fig-height: 4
ggplot(io_results, aes(color=format, linetype=late_materialization, x=selectivity, y=num_ios)) +
 geom_line() +
 facet_wrap(~ columns, scales = "free_y") +
 scale_x_continuous(label=scales::label_percent()) +
 theme(legend.position = "bottom")
