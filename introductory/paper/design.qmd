...
such as autonomous driving cars, LLMs, recommendation systems, RAG, and many others.
...
A significant aspect of AI workloads are that these workloads often have large columns.
...
Computer vision workloads often prefer that images are stored along the metadata and these
images are large variable length binary columns that are already compressed.
...
Arrow's type system for its table format and Lance libraries use Arrow as the in-memory
representation.
...
a format suitable for storage.  Compared with other columnar formats Lance files have a
stronger emphasis on sub-linear random access and, as a result, use simpler encodings and
avoid compression.  Lance files are also designed to support small row groups with as little
overhead as possible.

### Overall Structure

A Lance file starts with a sequence of row groups.  Each row group consists of a sequence
of columns.  After the last row group there is a metadata block containing the page table
and page statistics.  After this there is an optional manifest block which contains the
schema of the data.  Finally, there is a footer, which contains the file version, the position
of the metadata block, and a magic number.
...
The first 1024 rows of this dataset, in the Arrow in-memory layout, occupy X bytes of RAM.
This is equivalent to Y rows of TPC-H data.  Row group sizes of 1024 are not uncommon when
