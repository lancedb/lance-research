...
Details of the table format are beyond the scope of this paper but the following is a
brief overview of the key structures for holding data.

Tables consist of fragments.  Each fragment has one or more data files and an optional
deletion vector.  All data files have the same number of rows but different columns.
This allows columns to be added to an existing table without a rewrite.  A row addressing
scheme is needed to allow secondary index lookups.  A row address is a 64-bit identifier.
The upper 32 bits define the fragment id and the lower 32 bits define the fragment offset.
The row ids are not physically materialized.

![Lance Table Format](../figures/lance_table.png)
...
![Lance File Structure](../figures/lance_v1_file.png)
...
that support sub-linear random access which rules out generalized compression.  As a
result, Lance currently only supports a few simple encodings.  Other columnar formats
will divide column data into contiguous pages or compression chunks.  Since the currently
supported encodings do not benefit from this there is no similar concept in Lance.  Each
column can be thought of as a single page of data and we use the terms "column" and "page"
interchangeably. 
...
The first part of the metadata is the page table.  This table contains the file offset
